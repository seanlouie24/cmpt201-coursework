...
void intialize_block(struct header *block, uint64_t size, struct header *next,
                     int fill_value) {
  block->size = size;
  block->next = next;
...
int main(void) {
  ...
  intialize_block(first_block, BLOCK_SIZE, NULL, 0);
  intialize_block(second_block, BLOCK_SIZE, first_block, 1);
