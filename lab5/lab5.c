...
int find_best_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr->next != NULL) {
    if (free_list_ptr->size < best_fit_size && free_list_ptr->size >= size) {
...
int find_worst_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr->next != NULL) {
    if (free_list_ptr->size > worst_fit_size && free_list_ptr->size >= size) {
...
// TODO: Write a pseudo-code algorithm for the coalescing contiguous free
// blocks.
//
// Set current to the start of the free_list
// While current and current->next are not NULL:
//    If current block and next block are next to each other in mem:
